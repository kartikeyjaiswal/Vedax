...
import {
  ...
  Settings, Shield, LogOut, Leaf, BarChart3, PenTool, FileText
} from 'lucide-react'
...
export default function Sidebar({ isOpen, setIsOpen }) {
  ...
  return (
    <>
      {/* Mobile overlay - optional bonus */}
      {isOpen && (
        <div
          className="lg:hidden fixed inset-0 top-16 bg-black/60 z-30 backdrop-blur-sm transition-opacity"
          onClick={() => setIsOpen(false)}
        />
      )}

      <aside
        className={`fixed left-0 top-16 h-[calc(100vh-4rem)] flex flex-col z-40 transition-all duration-300 ease-in-out overflow-hidden
          ${isOpen ? 'w-64 translate-x-0' : 'w-0 -translate-x-full lg:translate-x-0'}
          ${isOpen ? 'flex' : 'hidden lg:flex'}
        `}
      >
        <div
          className={`flex-1 m-3 glass-card flex flex-col overflow-hidden w-[calc(16rem-1.5rem)] min-w-[calc(16rem-1.5rem)] transition-opacity duration-300
        ...
        >
            ...
            <div className="p-4 border-b border-slate-700/50">
              <div className="flex items-center gap-3 mb-3">
                {userDoc.profileImage ? (
                  <img src={userDoc.profileImage} alt={userDoc.name} className="w-10 h-10 rounded-full object-cover shadow-sm" />
                ) : (
                  <div className="w-10 h-10 rounded-full bg-gradient-to-br from-eco-500 to-ocean-600 flex items-center justify-center font-bold text-white text-sm shadow-sm">
            ...
            <div className="p-4 border-b border-slate-700/50">
              <div className="flex items-center gap-3">
                {userDoc.profileImage ? (
                  ...
                  <div className="w-10 h-10 rounded-full bg-gradient-to-br from-purple-500 to-indigo-600 flex items-center justify-center font-bold text-white text-sm shadow-sm">
                    {userDoc.name?.charAt(0)?.toUpperCase()}
                  </div>
                )}
                <div className="flex-1 min-w-0">
                  <div className="font-semibold text-white text-sm truncate">{userDoc.name}</div>
                  <div className="text-xs text-purple-400 font-bold tracking-wider uppercase mt-0.5">Super Admin</div>
                </div>
              </div>
            </div>
          )}
          ...
          <nav className="flex-1 p-3 space-y-1 overflow-y-auto hide-scrollbar">
            ...
            {/* Admin links */}
            {userDoc?.role === 'college_admin' && (
              <>
                <div className="text-xs text-gray-500 uppercase tracking-wider px-3 pb-2 pt-4">Admin</div>
                <NavLink
                  to="/admin"
                  className={({ isActive }) =>
                    `flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all duration-200 ${isActive ? 'bg-ocean-500/15 text-ocean-400 border border-ocean-500/30' : 'text-gray-400 hover:text-white hover:bg-white/5'
                    }`
                  }
                >
            ...
            {userDoc?.role === 'super_admin' && (
              <NavLink
                to="/super-admin"
                className={({ isActive }) =>
                  `flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all duration-200 ${isActive ? 'bg-purple-500/15 text-purple-400 border border-purple-500/30' : 'text-gray-400 hover:text-white hover:bg-white/5'
                  }`
                }
              >
