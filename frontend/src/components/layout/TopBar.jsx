import { Bell, Moon, Sun, Search, Flame } from 'lucide-react'
...
function NotificationsDropdown({ userDoc }) {
  ...
  const { data } = useQuery({
    ...
    queryFn: () => tasksAPI.getAll({ limit: 4, collegeId: userDoc?.collegeId }),
  ...
  return (
    <div className="relative" ref={ref}>
      <button
        ...
        className="relative p-2 rounded-lg bg-slate-800/60 border border-slate-700/50 text-gray-400 hover:text-white transition-colors"
      >
        <Bell className="w-4 h-4" />
        <span className="absolute top-1 right-1 w-2 h-2 rounded-full bg-eco-500" />
      </button>
          ...
          >
            ...
            {tasks.length === 0 ? (
              ...
              <div className="space-y-2">
                {tasks.map(t => (
                  <div key={t.$id} className="p-3 bg-slate-800/40 rounded-xl hover:bg-slate-800/60 transition-colors border border-transparent hover:border-slate-700/50 cursor-pointer">
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        <div className="text-sm font-medium text-white line-clamp-1">New task: {t.title}</div>
                        <div className="text-xs text-gray-400 mt-0.5">Earn {t.points} XP • Added recently</div>
                      </div>
                    </div>
                  </div>
                ))}
  ...
  return (
    <div className="flex items-center justify-between gap-4 max-w-7xl mx-auto">
      ...
      <div className="flex items-center gap-2 md:gap-3">
        ...
        {/* Avatar */}
        {userDoc && (
          <Link to="/profile" className="w-8 h-8 rounded-full bg-gradient-to-br from-eco-500 to-ocean-600 flex items-center justify-center font-bold text-white text-sm cursor-pointer hover:shadow-glow-eco hover:scale-105 transition-all">
